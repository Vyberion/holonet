...
function reportPreviewEmbed({ title, scope, weekStart, authorName = "", members = [], prefilled = false }) {
  ...
  const visibleRows = rows.slice(0, REPORT_PREVIEW_LIMIT).map(member => {
    ...
    return `ΓÇó **${memberName(member)}** ΓÇö ${formatHoursMinutesFromMinutes(normalizeMemberMinutes(member))}${events.length ? ` (${events.join(", ")})` : ""}`;
  });

  if (rows.length > REPORT_PREVIEW_LIMIT) {
    visibleRows.push(`ΓÇªand ${rows.length - REPORT_PREVIEW_LIMIT} more.`);
  }
...
export async function handleButton(interaction) {
  ...
  if (interaction.customId.startsWith("we_post:")) {
    ...
    if (!channel?.isTextBased?.() || typeof channel.send !== "function") {
      return interaction.update(ephemeral(componentsV2Message([containerV2([textDisplayV2(`Γ¥î Could not access target channel <#${channelId}>.`)])])));
    }
