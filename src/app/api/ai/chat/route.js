  ...
  try {
    ...
    if (toolName === "get_shift_totals") {
      ...
      if (includeHistoricalReports) {
        let reportQuery = "division_weekly_reports?status=eq.published&select=*,division_weekly_report_members(*)";
        if (scope !== "all") reportQuery += `&division_key=eq.${encodeURIComponent(scope)}`;
        if (timeframe === "month") {
