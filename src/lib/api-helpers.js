...
export function isMissingSchemaError(error) {
  return /does not exist|Could not find the table|column .* does not exist|schema cache|bucket not found/i.test(String(error?.message || ""));
}
...
export async function normalizeRows(resources, detailRows, resourceType) {
  ...
  return Promise.all(resources.map(async resource => {
    ...
    if (resourceType === "handbook") {
      ...
      const googleKind = googleWorkspaceKindFromUrl(googleDocUrl);
      const sourceType = googleKind === "spreadsheet"
        ? "google_spreadsheet"
...
export function buildPersonnelRankIndex(allRosters) {
  ...
  const divHRThresholds = {
    reavers: 15,        // Reaver Lord (15), Reaver Commander (200)
    dhg: 80,            // Guard Lieutenant (80), Guard Captain (90), Guard Commander (100)
    inquisitors: 155,   // High Inquisitor (155), Grand Inquisitor (200)
    dreadmasters: 10    // Dread Masters (10, 15, 20, 25, 30, 36)
  };

  const divMRThresholds = {
    reavers: [10],      // Senior Reaver
    dhg: [60],          // Senior Guard
    inquisitors: [150], // Senior Inquisitor
    dreadmasters: [4]   // Dread Captain
  };
...
export function computeRankBracketStatistics(activeUsers, rankIndex, rankFilter = "all_hr", timeframeLabel = "This Week") {
  const fullRosterMembers = Array.from(rankIndex.allMembers.values()).filter(m => matchesRankFilter(m, rankFilter));
  ...
  for (const member of fullRosterMembers) {
    ...
    if (matchedActive && matchedActive.totalHours > 0) {
      ...
      totalMinutes += matchedActive.totalMinutes;
      ...
      const activeRecord = {
        ...
        minutes: matchedActive.totalMinutes,
        shiftCount: matchedActive.shiftCount,
        onDutyNow: matchedActive.onDutyNow
      };
